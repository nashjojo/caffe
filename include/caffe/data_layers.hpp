 ...
 * Top:     Mapped Data from Memory
 * Bottom:  ID to map data from Memory
 * 
 * TODO(dox): thorough documentation for Forward and proto params.
 */
template <typename Dtype>
class MemoryMappingDataLayer : public Layer<Dtype> {
 public:
  explicit MemoryMappingDataLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual ~MemoryMappingDataLayer() {};
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
  // Data layers have only 1 bottoms, so reshaping is trivial.
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {}
  
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_MEMORY_MAPPING_DATA;
  }
